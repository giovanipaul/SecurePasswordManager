package io.github.giovanipaul.securepasswordmanager.service;
...
public class VaultService {
	...
	public List<VaultEntry> list(Vault vault) {
		return vault.getEntries().stream()
				.sorted(Comparator.comparing(VaultEntry::getSite, String.CASE_INSENSITIVE_ORDER)).toList();
	}

	public Optional<VaultEntry> getById(Vault vault, String id) {
		if (id == null || id.isBlank())
			return Optional.empty();
		return vault.getEntries().stream().filter(e -> id.equals(e.getId())).findFirst();
	}
	...
	public List<VaultEntry> search(Vault vault, String query) {
		...
		return list(vault)
				.stream().filter(entry -> contains(entry.getSite(), normalized)
						|| contains(entry.getUsername(), normalized) || contains(entry.getNotes(), normalized))
				.toList();
	}
	...
	public boolean update(Vault vault, String id, String site, String username, String password, String notes) {
		...
		if (site != null)
			e.setSite(nextSite);
		if (username != null)
			e.setUsername(nextUsername);
		if (password != null)
			e.setPassword(nextPassword);
		if (notes != null)
			e.setNotes(normalizeOptional(notes));
		return true;
	}
	...
	private static void ensureUnique(Vault vault, String site, String username, String excludedId) {
		...
		if (duplicate) {
			throw new IllegalArgumentException("An entry already exists for this service and username.");
		}
	}

...
class CryptoServiceTest {
	...
	@Test
	void authenticatesEnvelopeMetadata() throws Exception {
		byte[] salt = crypto.randomSalt();
		byte[] iv = crypto.randomIv();
		SecretKey key = crypto.deriveKey("CorrectMaster1!".toCharArray(), salt);
		byte[] plaintext = "secret".getBytes(StandardCharsets.UTF_8);
		byte[] ciphertext = crypto.encrypt(plaintext, key, iv, "version=2".getBytes(StandardCharsets.UTF_8));

		assertArrayEquals(plaintext, crypto.decrypt(ciphertext, key, iv, "version=2".getBytes(StandardCharsets.UTF_8)));
		assertThrows(AEADBadTagException.class,
				() -> crypto.decrypt(ciphertext, key, iv, "version=3".getBytes(StandardCharsets.UTF_8)));
	}
}
